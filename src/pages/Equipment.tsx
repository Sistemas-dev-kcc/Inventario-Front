...
function Equipment() {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      await api(`/equipment/${equipmentToDelete.id}`, {
        method: "DELETE",
      });
  ...
  function getTypeLabel(equipmentType: EquipmentTypeEnum) {
  ...
  function getTypeIcon(equipmentType: EquipmentTypeEnum) {
    switch (equipmentType) {
      case "LAPTOP":
        return <LaptopIcon />;
  ...
  function getStatusConfig(equipmentStatus: EquipmentStatus) {
    ...
    <div className="min-h-full bg-slate-50 p-4 sm:p-6 lg:p-8">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center px-6 py-16">
            ...
            <p className="mt-4 text-sm text-slate-500">Cargando equipos...</p>
          </div>
        )}
      ...
      <ConfirmModal
        ...
        message={
          deleteError
            ? deleteError
            : `¿Estás seguro de eliminar a ${equipmentToDelete?.hostname}? Esta acción no se puede deshacer.`
        }
      ...
      <AssignEquipmentModal
        ...
        onAssigned={async () => {
          ...
          if (selectedEquipment) {
            const updated = await api<EquipmentType>(
...
interface EquipmentCardProps {
  ...
  getTypeIcon: (type: EquipmentTypeEnum) => React.ReactNode;
  getTypeLabel: (type: EquipmentTypeEnum) => string;
  getStatusConfig: (status: EquipmentStatus) => {
    label: string;
    className: string;
    dot: string;
  };
  ...
  onDelete: (item: EquipmentType) => void;
}
  ...
  return (
    ...
    >
      ...
      <div className="mt-5 space-y-3">
        <InfoRow label="Modelo" value={item.model || "Sin modelo"} />

        <InfoRow label="Serial" value={item.serialNumber} mono />

        <InfoRow label="IP" value={item.ip || "Sin IP"} mono />

        <InfoRow
          label="Sistema"
          value={item.operatingSystem || "Sin sistema operativo"}
        />
      </div>
      ...
      >
        ...
        >
          ...
          <span className="xs:hidden">{item.user ? "Cambiar" : "Asignar"}</span>
        </button>
...
function AssignIcon() {
  return (
    ...
    >
      ...
      <path strokeLinecap="round" strokeLinejoin="round" d="M16 11h6M19 8v6" />
    </svg>
  );
}
